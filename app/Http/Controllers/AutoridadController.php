...
class AutoridadController extends Controller
{
    public function index(Request $request)
    {
        ...
        $tipoRevision = $request->input('tipo_autorizacion'); // E.g., 'proyectos'
        $filtroSubsector = $request->input('subsector');
        $filtroEstado = $request->input('estado_autoridad');
        ...
        $estadosAutoridad = array_map(fn($case) => $case instanceof \BackedEnum ? $case->value : $case, ['pendiente', 'Aprobado', 'Devuelto']);
        if ($tipoRevision) {
            $modelos = [
            ...
            if (array_key_exists($tipoRevision, $modelos)) {
                ...
                // Aplicar Filtro por Estado de AUTORIDAD
                if ($filtroEstado) {
                    $query->where('estado_autoridad', $filtroEstado);
                }
