...
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-bold mb-4">Autorización de Planes, Programas y Proyectos</h1>
                    ...
                    <tbody>
                        @foreach ($$tipo as $item)
                            ...
                            <tr class="border-b">
                                ...
                                <td class="border p-2">{{ $item->estado_autoridad }}</td>
                                <td class="border p-2">
                                ...
                                <td class="border p-2">
                                    <form action="{{ route('autoridad.estado', ['tipo' => $tipo, 'id' => $item->$primaryKey]) }}" method="POST" class="flex items-center space-x-2">
                                        @csrf
                                        @method('PUT')
                                        <select name="estado_autoridad" class="border rounded px-2 py-1">
                                            <option value="pendiente" {{ $item->estado_autoridad == 'pendiente' ? 'selected' : '' }}>pendiente</option>
                                            <option value="Aprobado" {{ $item->estado_autoridad == 'Aprobado' ? 'selected' : '' }}>Aprobado</option>
                                            <option value="Devuelto" {{ $item->estado_autoridad == 'Devuelto' ? 'selected' : '' }}>Devuelto</option>
                                        </select>
...
<a href="{{route('dashboard.autoridad')}}" class="font-bold btn btn-secondary text-white">VOLVER</a>
@endsection