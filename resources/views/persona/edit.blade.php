...
<form action="{{ route('persona.update', $usuario->idUser) }}" method="POST" class="space-y-4">
    ...
    <div class="mb-4">
        <input type="text" name="cedula" class="w-full border rounded p-2" placeholder="Cédula"
               value="{{ old('cedula', $usuario->cedula) }}" required>
    </div>

    <div class="mb-4">
        <input type="text" name="name" required class="w-full border rounded p-2" placeholder="Nombre"
               value="{{ old('name', $usuario->name) }}" >
    </div>

    <div class="mb-4">
        <input type="text" name="apellidos" required class="w-full border rounded p-2" placeholder="Apellidos"
               value="{{ old('apellidos', $usuario->apellidos) }}" >
    </div>

    <div class="mb-4">
        <input type="email" name="email" required class="w-full border rounded p-2" placeholder="Correo"
               value="{{ old('email', $usuario->email) }}">
    </div>

    <div class="mb-4">
        <input type="text" name="telefono" required class="w-full border rounded p-2" placeholder="Teléfono"
...
<input type="password" name="password" class="w-full mb-2 border rounded p-2" placeholder="Contraseña" required>
    <div class="flex gap-4">
        ...
        <a href="{{ route('persona.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-700">CANCELAR</a>
    </div>
</form>
 </div>
@endsection