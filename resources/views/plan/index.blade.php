    ...
    <table class="min-w-full table-auto border-collapse">
        ...
        <tr>
            ...
            <td class="p-2 flex gap-2">
                {{-- Enlace para Editar --}}
                <a href="{{ route('plan.edit', $p->idPlan) }}" 
                class="inline-flex items-center px-3 py-1.5 bg-yellow-500 text-white text-xs font-bold rounded hover:bg-yellow-600 transition duration-150 shadow-sm" title="Editar plan">
                    <i class="fas fa-edit mr-2"></i>
                    Editar
                </a>
                {{-- Formulario para Eliminar --}}
                <form action="{{ route('plan.destroy', $p->idPlan) }}" method="POST" 
                class="inline" 
                onsubmit="return confirmDelete(event, '{{addslashes($p->nombre)}}')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
                            class="inline-flex items-center px-3 py-1.5 bg-red-600 text-white text-xs font-bold rounded hover:bg-red-700 transition duration-150 shadow-sm" title="Eliminar plan">
...
<a href="{{ route('dashboard.externo') }}" class="font-bold bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">REGRESAR</a> 
    ...
    <script>
        function confirmDelete(event, nombre) {
            ...
            Swal.fire({
                title: '¿Está seguro?',
                html: `Esta acción eliminará permanentemente el plan:<br><strong>"${nombre}"</strong><br>y no podrá revertirse`,
                icon: 'warning',
