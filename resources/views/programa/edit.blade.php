        ...
        <div id="tabContents">
            <div class="tab-content" id="tab0">
                <div class="mb-4">
                    ...
                    <select name="tipo_dictamen" id="tipo_dictamen" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="objeto" id="objeto" class="w-full border rounded p-2" value="{{ $programa->objeto }}" required>
                </div>
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="plazo_ejecucion" class="w-full border rounded p-2" value="{{ $programa->plazo_ejecucion }}" required>
                </div>
                <div class="mb-4">
                    <label class="font-bold">1.6 Monto total</label>
                    <input type="text" name="monto_total" class="w-full border rounded p-2" value="{{ $programa->monto_total }}" required>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 gap-2">
                        @foreach($objetivosEstrategicos as $objetivo)
                            <label class="inline-flex items-center space-x-2">
                                <input type="checkbox" name="idObjetivoEstrategico[]" value="{{ $objetivo->idObjetivoEstrategico }}"
                ...
                <div class="mb-4">
                    ...
                    <div class="grid grid-cols-1 gap-2">
                        @foreach($metasEstrategicas as $meta)
                            <label class="inline-flex items-center space-x-2">
                                <input type="checkbox" name="idMetaEstrategica[]" value="{{ $meta->idMetaEstrategica }}"
            ...
            <div class="tab-content hidden" id="tab3">
                ...
                <div id="componentesContainer" class="space-y-4">
                    ...
                    <div class="componente border p-4 rounded shadow">
                        <div class="mb-2">
                            ...
                            <input type="text" name="componentesPrograma[{{ $ci }}][nombre]" class="w-full border rounded p-2" value="{{ $componente->nombre }}" required>
                        </div>
                        <div class="mb-2">
                        ...
                        <div class="mb-2">
                            ...
                            <input type="number" step="0.01" name="componentesPrograma[{{ $ci }}][monto]" class="w-full border rounded p-2 componente-monto" value="{{ $componente->monto }}" required>
                        </div>
                        ...
                        <div class="actividades space-y-2" data-componente-index="{{ $ci }}">
                            @foreach($componente->actividadesPrograma as $ai => $actividad)
                            <div class="actividad border p-3 rounded bg-gray-50">
                                <label class="font-semibold block mb-1">Actividad {{ $ai + 1 }}</label>
                                <input type="text" name="componentesPrograma[{{ $ci }}][actividadesPrograma][{{ $ai }}][nombre]" class="border p-2 w-full my-1" value="{{ $actividad->nombre }}" required>
                                <input type="number" step="0.01" name="componentesPrograma[{{ $ci }}][actividadesPrograma][{{ $ai }}][monto]" class="border p-2 w-full mb-2 actividad-monto" value="{{ $actividad->monto }}" required>
                                <div class="tareas space-y-1">
                                    @foreach($actividad->tareasPrograma as $ti => $tarea)
                                    <div class="tarea ml-4 border-l pl-2">
                                        <input type="text" name="componentesPrograma[{{ $ci }}][actividadesPrograma][{{ $ai }}][tareasPrograma][{{ $ti }}][nombre]" class="border p-1 w-full mb-1" value="{{ $tarea->nombre }}" required>
                                        <input type="number" step="0.01" name="componentesPrograma[{{ $ci }}][actividadesPrograma][{{ $ai }}][tareasPrograma][{{ $ti }}][monto]" class="border p-1 w-full mb-2" value="{{ $tarea->monto }}" required>
                                    </div>
                                    @endforeach
                                </div>
                                <button type="button" class="btn-tarea bg-blue-500 text-white px-2 py-1 rounded" data-comp="{{ $ci }}" data-act="{{ $ai }}">Agregar Tarea</button>
                            </div>
                            @endforeach
                        </div>
                        <button type="button" class="btn-actividad bg-blue-600 text-white px-3 py-1 rounded" data-componente-index="{{ $ci }}">Agregar Actividad</button>
                    </div>
                    @endforeach
                </div>
                ...
                <div id="cronogramaContainer" class="space-y-4"></div>
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    // Inicializar valores de componentes, actividades y tareas desde los datos del programa
    componenteIndex = {{ count($programa->componentesPrograma) }};
    // Generar cronograma y resumen inicial
    generarCronogramaDesdeComponentes();
    generarTablaResumen();
    actualizarSaldo();

    // Inicializar mapa con lat/lng del programa si existen
    const lat = parseFloat("{{ $programa->latitud }}") || -0.1807;
    const lng = parseFloat("{{ $programa->longitud }}") || -78.4678;
...
document.getElementById('addComponente').addEventListener('click', () => {
    ...
    nuevo.innerHTML = `
        <div class="mb-2">
            <label class="block text-sm font-bold">Nombre del Componente</label>
            <input type="text" name="componentesPrograma[${componenteIndex}][nombre]" class="w-full border rounded p-2" required>
        </div>
        <div class="mb-2">
            <label class="block text-sm font-bold">Descripción</label>
    ...
    // Cambios en nuevo input de monto
...
// Cambios en los inputs de monto de los componentes
...
//Generar cronograma desde componentes
...
document.querySelectorAll('.btn-actividad').forEach(btn => {
        btn.onclick = () => {
            ...
            const montoComponente = parseFloat(componenteEl.querySelector('input[name^="componentesPrograma"][name$="[monto]"]').value) || 0;
            let totalAsignado = 0;
            ...
            nuevaActividad.innerHTML = `
                ...
                <input type="text" name="componentesPrograma[${idx}][actividadesPrograma][${actividades.length}][nombre]" class="border p-2 w-full my-1" placeholder="Nombre de la actividad" required>
                <input type="number" step="0.01" name="componentesPrograma[${idx}][actividadesPrograma][${actividades.length}][monto]" class="border p-2 w-full mb-2 actividad-monto" placeholder="Monto de la actividad" required>
            ...
            activarBotonesTarea();
            nuevaActividad.querySelector('.actividad-monto').addEventListener('input', generarTablaResumen);
            generarTablaResumen();
        };
    });
}
    ...
    document.querySelectorAll('.btn-tarea').forEach(btn => {
        btn.onclick = () => {
            ...
            nuevaTarea.innerHTML = `
                ...
                <input type="text" name="componentesPrograma[${compIdx}][actividadesPrograma][${actIdx}][tareasPrograma][${tareas.length}][nombre]"
                    class="border p-1 w-full mb-1" placeholder="Nombre de la tarea" required>
                <input type="number" step="0.01" name="componentesPrograma[${compIdx}][actividadesPrograma][${actIdx}][tareasPrograma][${tareas.length}][monto]" 
                    class="border p-1 w-full mb-2" placeholder="Monto de la tarea" required>
            `;
            tareaContainer.appendChild(nuevaTarea);
            nuevaTarea.querySelector('input[name$="[monto]"]').addEventListener('input', generarTablaResumen);
            generarTablaResumen();
        };
    });
}
...
// Recalcular tabla siempre que haya cambios en cronograma o componentes
