...
@endif
<h2 class="text-2xl font-extrabold text-gray-800 mb-6 border-b pb-2"> EDITAR PROYECTO</h2>
    <div class="max-w-6xl mx-auto bg-white p-8 rounded-xl shadow-2xl">
        <form action="{{route ('proyecto.update', $proyecto->idProyecto )}}"method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            {{-- Barra de Pestañas --}}
                    ...
                    <div class="space-y-4">
                        <div class="bg-blue-50 p-4 rounded-lg">
                            ...
                            <select name="tipo_dictamen" id="tipo_dictamen" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                ...
                                <option value="prioridad" {{ $proyecto->tipo_dictamen == 'prioridad' ? 'selected' : '' }}>Dictamen de prioridad</option>
                                <option value="aprobacion" {{ $proyecto->tipo_dictamen == 'aprobacion' ? 'selected' : '' }}>Dictamen de aprobación</option>
                                <option value="actualizacion_prioridad" {{ $proyecto->tipo_dictamen == 'actualizacion_prioridad' ? 'selected' : '' }}>Actualización de prioridad</option>
                                <option value="actualizacion_aprobacion" {{ $proyecto->tipo_dictamen == 'actualizacion_aprobacion' ? 'selected' : '' }}>Actualización de aprobación</option>
                            </select>
                            <p class="text-xs text-blue-500 mt-1">Define el proposito de está solicitud ante la autoridad competente.</p>
                        </div>
                        <div class="bg-blue-50 p-4 rounded-lg">
                            <input type="hidden" name="nombre" id="nombre" value="{{ $proyecto->nombre }}">
                            ...
                            <select name="accion" id="accion" class="w-full border rounded p-2" required>
                                ...
                                @foreach(['adquisición', 'construcción', 'adecuación', 'ampliación', 'dotación', 'habilitación', 'instalación', 'mejoramiento', 'implementación', 'recuperación', 'rehabilitación', 'renovación', 'reparación', 'reposicion', 'investigación', 'generación de información', 'saneamiento'] as $accion)
                                    <option value="{{ $accion }}" {{ $proyecto->accion == $accion ? 'selected' : '' }}>{{ ucfirst($accion) }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="bg-blue-50 p-4 rounded-lg">
                            ...
                            <input type="text" name="objeto" id="objeto" class="w-full border rounded p-2" placeholder="Ej: carretera, hospital, unidad educativa" required value="{{ $proyecto->objeto }}">
                            <p class="text-xs text-blue-500 mt-1">El nombre del Proyecto se genera automáticamente: <span id="nombreProgramaPreview" class="font-semibold text-blue-500">...</span></p>
                        </div>
                    </div>
                    <div class="space-y-4">
                        <div class="bg-blue-50 p-4 rounded-lg">
                            ...
                            <select name="idEntidad" class="w-full border rounded p-2" required>
                                ...
                                @foreach ($entidad as $e)
                                    <option value="{{ $e->idEntidad }}" {{ $proyecto->idEntidad == $e->idEntidad ? 'selected' : '' }}>{{ $e->subSector }}</option>
                                @endforeach
                            </select>
                            ...
                            <input type="text" name="plazo_ejecucion" class="w-full border rounded p-2" required value="{{ $proyecto->plazo_ejecucion }}">
                        ...
                        <div class="bg-blue-50 p-4 rounded-lg">
                            ...
                            <input type="number" step="0.01" name="monto_total" class="w-full border rounded p-2" required value="{{ $proyecto->monto_total }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        <div class="mb-6 bg-blue-50 p-4 rounded-lg">
                            ...
                            <textarea name="diagnostico" class="w-full border rounded p-2" rows="5">{{ $proyecto->diagnostico }}</textarea>
                        </div>
                    </div>
                    <div class="space-y-4">
                        <div class="bg-blue-50 p-4 rounded-lg">
                            <label class="font-bold block mb-2">2.2 Identificación, descripción y diagnóstico del problema</label>
                            <textarea name="problema" class="w-full border rounded p-2" rows="5">{{ $proyecto->problema }}</textarea>
                        </div>
                    </div>
                </div>
                <div class="mb-4">
                    ...
                    <div class="bg-blue-50 p-4 rounded-lg">
                        <label>Latitud</label>
                        <input type="text" name="latitud" id="latitud" value="{{ $proyecto->latitud }}">
                        <label>Longitud</label>
                        <input type="text" name="longitud" id="longitud" value="{{ $proyecto->longitud }}">
                   </div>
                </div>
            {{-- Pestaña 2: Alineación --}}
            <div class="tab-content hidden" id="tab2">
                <div class="mb-8 bg-blue-50 p-5 rounded-lg border border-blue-200">
                    <label class="font-bold text-blue-700 mb-3 text-lg">Objetivos Estratégicos</label>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        @foreach($objetivoEstrategico as $objetivo)
                            <label class="inline-flex items-start space-x-3 cursor-pointer p-2 bg-white rounded shadow-sm hover:bg-blue-100 transition-colors">
                                ...
                                {{ in_array($objetivo->idObjetivoEstrategico, $proyecto->objetivosEstrategicos->pluck('idObjetivoEstrategico')->toArray() ?? []) ? 'checked' : '' }}>
                                <span class="text-sm text-gray-700">{{ $objetivo->descripcion }}</span>
                            </label>
                        @endforeach
                    </div>
                    <p class="text-xs text-blue-500 mt-3">Seleccione todos los objetivos estratégicos a los que el proyecto contribuye.</p>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        @foreach($metasEstrategicas as $meta)
                            <label class="inline-flex items-start space-x-3 cursor-pointer p-2 bg-white rounded shadow-sm hover:bg-green-100 transition-colors">
                                ...
                                {{ in_array($meta->idMetaEstrategica, $proyecto->metasEstrategicas->pluck('idMetaEstrategica')->toArray() ?? []) ? 'checked' : '' }}>
                                <span class="text-sm text-gray-700">{{ $meta->descripcion }}</span>
                            </label>
                        @endforeach
                    </div>
                    <p class="text-xs text-green-500 mt-3">Indique las metas específicas que serán impactadas por el proyecto.</p>
                </div>
            </div>
            ...
            <div class="tab-content hidden" id="tab3">
                ...
                <div class="flex justify-start space-x-8 mb-6 p-4 bg-indigo-50 rounded-lg shadow-inner">
                    <div class="mb-4">
                        ...
                        <span id="montoTotalDisplay" class="text-blue-600 font-bold">${{ number_format($proyecto->monto_total, 2) }}</span>
                    </div>
                    <div class="mb-4">
                ...
                <div id="componentesContainer" class="space-y-4">
                    @foreach($proyecto->componentesProyecto as $ci => $componente)
                    <div class="componente border p-4 rounded shadow" data-componente-index="{{ $ci }}">
                        <input type="hidden" name="componentesProyecto[{{ $ci }}][idComponenteProyecto]" value="{{ $componente->idComponenteProyecto ?? '' }}">
                        <div class="mb-2">
                            <label class="block text-sm font-bold">Nombre del Componente</label>
                            <input type="text" name="componentesProyecto[{{ $ci }}][nombre]" class="w-full border rounded p-2 componente-nombre" required value="{{ $componente->nombre }}">
                        </div>
                        <div class="mb-2">
                            <label class="block text-sm font-bold">Descripción</label>
                            <textarea name="componentesProyecto[{{ $ci }}][descripcion]" class="w-full border rounded p-2" rows="3">{{ $componente->descripcion }}</textarea>
                        </div>
                        <div class="mb-2">
                            <label class="block text-sm font-bold">Monto asignado</label>
                            <input type="number" step="0.01" name="componentesProyecto[{{ $ci }}][monto]" class="w-full border rounded p-2 componente-monto" required value="{{ $componente->monto }}">
                        </div>
                ...
                <div id="cronogramaContainer" class="space-y-4">
                    ...
                    <div class="cronograma-bloque border p-4 rounded shadow" data-componente-index="{{ $ci }}">
                        ...
                        <div class="actividades space-y-2 mb-4" data-componente-index="{{ $ci }}">
                            @foreach($componente->actividadesProyecto as $ai => $actividad)
                            <div class="actividad border p-3 rounded bg-gray-50" data-actividad-index="{{ $ai }}">
                                <input type="hidden" name="componentesProyecto[{{ $ci }}][actividadesProyecto][{{ $ai }}][idActividadProyecto]" value="{{ $actividad->idActividadProyecto ?? '' }}">
                                ...
                                <input type="text" name="componentesProyecto[{{ $ci }}][actividadesProyecto][{{ $ai }}][nombre]" class="border p-2 w-full my-1 actividad-nombre" placeholder="Nombre de la actividad" required value="{{ $actividad->nombre }}">
                                <input type="number" step="0.01" name="componentesProyecto[{{ $ci }}][actividadesProyecto][{{ $ai }}][monto]" class="border p-2 w-full mb-2 actividad-monto" placeholder="Monto de la actividad" required value="{{ $actividad->monto }}">
                            </div>
                            @endforeach
                        </div>
        ...
        <div class="flex justify-between pt-6">
            ...
            <a href="{{ route('proyecto.index') }}" class="btn btn-secondary font-bold bg-gray-500 text-white p-2 rounded hover:bg-gray-600">Volver</a>
        </div>
    </form>
</div>
<script>
document.addEventListener('DOMContentLoaded', function () {
    ...
    let componenteIndex = {{ count($proyecto->componentesProyecto) }};
    function actualizarSaldoProyecto() {
        ...
        componentes.forEach(compEl => {
            ...
            if (sumaAsignada + val > montoTotal) {
                ...
                alert('El monto asignado al componente no puede superar el monto total disponible del proyecto.');
            }
    ...
    function agregarNuevoComponente() {
        ...
        nuevo.innerHTML = `
            <div class="mb-2">
                <label class="block text-sm font-bold">Nombre del Componente</label>
                <input type="text" name="componentesProyecto[${ci}][nombre]" class="w-full border rounded p-2 componente-nombre" required>
            </div>
            <div class="mb-2">
                <label class="block text-sm font-bold">Descripción</label>
                <textarea name="componentesProyecto[${ci}][descripcion]" class="w-full border rounded p-2" rows="3"></textarea>
            </div>
            <div class="mb-2">
                <label class="block text-sm font-bold">Monto asignado</label>
                <input type="number" step="0.01" name="componentesProyecto[${ci}][monto]" class="w-full border rounded p-2 componente-monto" required>
            </div>
            <button type="button" class="remove-componente mt-2 px-3 py-1 bg-red-600 text-white rounded text-sm">Eliminar Componente</button>
        `;
    ...
    function agregarNuevaActividad(componenteIndex) {
        ...
        nuevaActividad.innerHTML = `
            ...
            <input type="text" name="componentesProyecto[${componenteIndex}][actividadesProyecto][${actividadIndex}][nombre]" class="border p-2 w-full my-1 actividad-nombre" placeholder="Nombre de la actividad" required>
            <input type="number" step="0.01" name="componentesProyecto[${componenteIndex}][actividadesProyecto][${actividadIndex}][monto]" class="border p-2 w-full mb-2 actividad-monto" placeholder="Monto de la actividad" required>
        `;
        ...
        componentes.forEach((compEl, newIndex) => {
            ...
            compEl.querySelectorAll('input, textarea').forEach(input => {
                input.name = input.name.replace(`componentesProyecto[${oldIndex}]`, `componentesProyecto[${newIndex}]`);
            });
        ...
        actividades.forEach((actEl, newIndex) => {
            ...
            actEl.querySelectorAll('input').forEach(input => {
                input.name = input.name.replace(`[actividadesProyecto][${oldIndex}]`, `[actividadesProyecto][${newIndex}]`);
            });
        });
    }
    ...
    // 2. Generación del Nombre del Proyecto
    ...
    // Listener principal para todo el contenedor de componentes (actualiza saldos del proyecto y cronograma)
    ...
    document.querySelector('input[name="monto_total"]').addEventListener('input', actualizarSaldoProyecto);
