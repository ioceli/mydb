        ...
        <tr>
            ...
            <td class="border p-2">{{ $p->estado_autoridad }}</td>
                    <td class="p-2 flex gap-2">
                        {{-- Enlace para Editar --}}
                <a href="{{ route('proyecto.edit', $p->idProyecto) }}" 
                class="inline-flex items-center px-3 py-1.5 bg-yellow-500 text-white text-xs font-bold rounded hover:bg-yellow-600 transition duration-150 shadow-sm" title="Editar proyecto">
                    <i class="fas fa-edit mr-2"></i>
                    Editar
                </a>
                {{-- Formulario para Eliminar --}}
                <form action="{{ route('proyecto.destroy', $p->idProyecto) }}" method="POST" 
                class="inline" 
                onsubmit="return confirmDelete(event, '{{addslashes($p->nombre)}}')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
                            class="inline-flex items-center px-3 py-1.5 bg-red-600 text-white text-xs font-bold rounded hover:bg-red-700 transition duration-150 shadow-sm" title="Eliminar proyecto">
...
<div class="mt-6">
    ...
    <script>
        function confirmDelete(event, nombre) {
            ...
            Swal.fire({
                title: '¿Está seguro?',
                html: `Esta acción eliminará permanentemente el proyecto:<br><strong>"${nombre}"</strong><br>y no podrá revertirse`,
                icon: 'warning',
